...
window.addEventListener('popstate', (event) => {
    ...
    // Jika state-nya player, kita biarkan atau handle khusus (opsional)
    // Tapi logic dasar: kembali ke tab sebelumnya
    if (state && state.page && state.page !== 'player') {
        switchTab(state.page, false);
    } else {
        switchTab('home', false);
    }
});
...
async function handleSearch() {
    ...
    // Sembunyikan semua kecuali search
    const pages = ['homePage', 'dubbedPage', 'vipPage', 'historyPage', 'playerPage'];
    pages.forEach(p => document.getElementById(p).style.display = 'none');
...
// --- PLAYER (FIX BUG VIDEO NUMPUK) ---
async function openPlayer(bookId) {
    // 1. SEMBUNYIKAN SEMUA HALAMAN LAIN (Fix Utama)
    const pages = ['homePage', 'dubbedPage', 'vipPage', 'historyPage', 'searchPage'];
    pages.forEach(p => document.getElementById(p).style.display = 'none');

    // 2. TAMPILKAN PLAYER
    document.getElementById('playerPage').style.display = 'block';
    
    // 3. SCROLL KE ATAS (Biar gak di bawah)
    window.scrollTo(0, 0);

    // 4. PUSH HISTORY (Biar bisa di-back)
