...
function goHome(pushHistory = true) {
    ...
    document.querySelectorAll('.cat-btn').forEach(b => {
        if(!b.classList.contains('vip-btn')) b.classList.remove('active');
    });
...
// --- LOAD VIP (PERBAIKAN LOGIKA DATA) ---
...
function renderBooks(list, containerId) {
    ...
    list.forEach(book => {
        ...
        if (book.corner && book.corner.name) {
            badgeHtml = `<div class="card-badge" style="background:${book.corner.color}">${book.corner.name}</div>`;
        }
        div.innerHTML = `
