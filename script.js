...
window.addEventListener('popstate', (event) => {
    ...
    if (state && state.page) {
        ...
        else if (state.page === 'vip') openVIPPage(false); // Restore VIP
...
// --- FUNGSI UTAMA HOME (TAMPILKAN TRENDING) ---
...
// --- FUNGSI BUKA HALAMAN VIP (TOGGLE) ---
function openVIPPage(pushHistory = true) {
    ...
    // Update Nav Active
    document.querySelectorAll('.cat-btn').forEach(b => b.classList.remove('active'));
    // (Opsional: Cari tombol VIP dan aktifkan classnya jika perlu)
...
async function initApp() {
    ...
    await loadHomeVIP();    // Load data VIP (tapi container hidden)
...
// --- LAINNYA (Search, Category, Player) ---
...
// --- RENDER BOOKS (MODIF: Label Gratis) ---
function renderBooks(list, containerId) {
    ...
    list.forEach(book => {
        ...
        // LOGIKA GANTI NAMA BADGE (VIP -> GRATIS)
        if (book.corner && book.corner.name) {
            let label = book.corner.name;
            if (label === "Anggota Saja") label = "Gratis"; // Ubah disini
            
            badgeHtml = `<div class="card-badge" style="background:${book.corner.color}">${label}</div>`;
