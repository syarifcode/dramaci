const Core = {
    ...
    // ANTI BUG STATE
    activeFetchId: 0,
    currentSource: null, // BARU: Untuk melacak sumber
    ...
    openPlayer: async (source, id) => {
        ...
        Core.currentSource = source; // SIMPAN SUMBER
        ...
        video.removeAttribute('crossorigin'); // Reset default
        ...
        if (Core.activeFetchId !== thisRequestId) { console.log("Request kadaluarsa."); return; }
    ...
    playSpecificEpisode: (index) => {
        ...
        // --- SMART SWITCH (LOGIKA PENTING) ---
        if (Core.currentSource === 'ns') {
            video.setAttribute('crossorigin', 'anonymous'); // Netshort butuh ini
        } else {
            video.removeAttribute('crossorigin'); // Dramabox ALERGI ini
        }

        video.src = ep.url;
        
        // Pasang Subtitle (Hanya Netshort)
        video.querySelectorAll('track').forEach(t => t.remove());
