const Core = {
    ...
    init: () => {
        ...
        // Scroll Explore (Cuma Load Sekali kalau Netshort biasanya static)
    ...
    // --- HOME LOGIC (DRAMABOX + MELOLO + NETSHORT) ---
    loadHome: async () => {
        try {
            // Ambil Data Buat Banner (Dramabox + Melolo)
            ...
            if (meloloData.status === 'fulfilled' && meloloData.value) {
                ...
                if (mCombined.length > 0) {
                    sliderData = [...sliderData, ...mCombined.slice(0, 2)];
            ...
            if (sliderData.length > 0) {
                sliderData.sort(() => Math.random() - 0.5); 
                Core.renderSlider(sliderData.slice(0, 5));
            }
    ...
    // --- EXPLORE LOGIC (KEMBALI KE V11: NETSHORT ONLY) ---
    loadExplore: async () => {
        const container = document.getElementById('exploreGrid');
        // Kalau sudah ada isinya, gak usah load lagi (Netshort biasanya sekali load semua)
        ...
        try {
            // MURNI AMBIL DARI NETSHORT SAJA
            ...
            if (data && data.viral && data.viral.length > 0) {
                ...
                container.innerHTML = '<p style="text-align:center;width:100%">Gagal memuat data explore.</p>';
            }

        } catch(e) { 
            console.error(e); 
            if(spinner) spinner.remove(); 
            container.innerHTML = '<p style="text-align:center;width:100%">Terjadi kesalahan jaringan.</p>';
        }
