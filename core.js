const Core = {
    ...
    loadHome: async () => {
        ...
        // Safety: Pastikan section melolo hilang
        const meloloSec = document.getElementById('homeMeloloSection');
        if(meloloSec) meloloSec.style.display = 'none';
    },
    ...
    // --- EXPLORE LOGIC (FIX DUPLICATE) ---
    loadExplore: async () => {
        if(Core.exploreLoading) return;
        Core.exploreLoading = true;
        
        const container = document.getElementById('exploreGrid');
        // Ambil ID yang sudah ada di layar agar tidak double
        ...
        try {
            ...
            // Filter: Hanya ambil data yang ID-nya BELUM ada di existingIds
    ...
    openPlayer: async (source, id) => {
        ...
        video.poster = details.cover; document.getElementById('detailSource').innerText = source === 'db' ? 'Icisa VIP' : 'Netshort';
    ...
    playSpecificEpisode: async (index) => {
        ...
        if (index < Core.currentEpisodeList.length - 1) { nextBtn.disabled = false; nextBtn.innerHTML = `Next <i class="fas fa-forward"></i>`; } 
        else { nextBtn.disabled = true; nextBtn.innerHTML = `Tamat <i class="fas fa-check"></i>`; }

        // --- Setup Tombol Download Langsung ---
    ...
    downloadCurrentEpisode: async (url, filename, btnElement) => {
        ...
        try {
            ...
            // Fallback ke Tab Baru jika CORS memblokir Fetch
    ...
    handleSearch: async () => {
        ...
        try {
            const [resDB, resNS] = await Promise.allSettled([
