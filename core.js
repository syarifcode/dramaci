const Core = {
    ...
    openSectionPage: async (provider) => {
        switchTab('sectionPage', true); // Panggil fungsi switchTab yang sudah diperbaiki di bawah
        ...
        try {
            if (provider === 'flick') {
                ...
                data = res.list || [];
                const extra = await Flick.search("CEO"); // Tambahan biar banyak
                data = [...data, ...extra];
            } 
        ...
        if (data.length > 0) {
            ...
            gridEl.innerHTML = '<p style="text-align:center;width:100%;margin-top:20px">Gagal memuat data / Data kosong.</p>';
        }
    },
...
// --- FUNGSI NAVIGASI YANG SUDAH DIPERBAIKI (V11.1) ---
function switchTab(t, p=true) {
    ...
    // 3. Reset Navigasi Bawah
    document.querySelectorAll('.nav-item').forEach(el => el.classList.remove('active'));

    // 4. Buka Halaman Tujuan (Termasuk SectionPage)
    if (t === 'home') {
        ...
        Core.loadHistory();
    } else if (t === 'sectionPage') {
        // INI YANG SEBELUMNYA HILANG
        document.getElementById('sectionPage').style.display = 'block';
        // Halaman section tidak punya menu bawah aktif (biar fokus konten)
    } else if (t === 'searchPage') {
        document.getElementById('searchPage').style.display = 'block';
    }
