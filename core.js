const Core = {
    ...
    init: () => {
        ...
        window.addEventListener('scroll', () => { 
            // Cek apakah sedang di halaman explore
            if (document.getElementById('explorePage').style.display !== 'none') { 
                // Rumus trigger scroll bawah
    ...
    // --- HOME LOGIC (FIX BANNER MELOLO) ---
    loadHome: async () => {
        try {
            // Ambil Data Melolo & Dramabox
            const [dbData, meloloData] = await Promise.allSettled([
                DramaboxV2.fetchTrending(),
                Melolo.fetchHome()
            ]);

            let sliderData = [];
            
            // 1. PRIORITAS MELOLO (Masuk Slider Duluan)
            if (meloloData.status === 'fulfilled' && meloloData.value) {
                const mData = meloloData.value;
                const mCombined = [...(mData.trending||[]), ...(mData.latest||[])];
                
                if (mCombined.length > 0) {
                    // Masukkan 3 drama Melolo ke kandidat slider
                    sliderData = [...sliderData, ...mCombined.slice(0, 3)];
                    // Render List Melolo
            ...
            // 2. DATA DRAMABOX
            if (dbData.status === 'fulfilled' && dbData.value && dbData.value.length > 0) {
                const dbList = dbData.value;
                // Masukkan 3 drama Dramabox ke kandidat slider
                sliderData = [...sliderData, ...dbList.slice(0, 3)]; 
                // Render Grid Trending
                Core.renderCards(dbList, 'homeGridDB'); 
            }

            // 3. RENDER SLIDER (Gabungan & Diacak)
            if (sliderData.length > 0) {
                sliderData.sort(() => Math.random() - 0.5); // Acak biar campur
                Core.renderSlider(sliderData.slice(0, 5)); // Ambil 5 teratas setelah diacak
            }
    ...
    // --- EXPLORE LOGIC (FIX INFINITY SCROLL V11) ---
    loadExplore: async () => {
        // Cek loading state biar gak double request
        if(Core.exploreLoading) return;
        Core.exploreLoading = true;
        
        const container = document.getElementById('exploreGrid');
        
        // Tampilkan Spinner di bawah (Append)
        const spinner = document.createElement('div');
        spinner.className = 'loading-spinner';
        container.appendChild(spinner);
        
        try {
            // Fetch Netshort (Selalu fetch ulang untuk simulasi infinity)
            const data = await Netshort.fetchTheaters();

            // Hapus spinner setelah data dapat
            spinner.remove();

            if (data && data.viral && data.viral.length > 0) {
                // Acak urutan biar terlihat beda setiap scroll
                const shuffled = data.viral.sort(() => 0.5 - Math.random());
                
                // APPEND (Tambah ke bawah) bukan replace
                // Kita izinkan duplikat ID karena ini "Infinity Scroll" acak
                // atau bisa difilter kalau mau strict
        ...
        // Lepas status loading biar bisa scroll lagi
        Core.exploreLoading = false;
    },

    // --- LOAD MORE HOME (DRAMABOX V2) ---
    ...
    openPlayer: async (source, id) => {
        ...
        document.getElementById('detailSource').innerText = source === 'db' ? 'Icisa VIP' : (source === 'ns' ? 'Netshort' : 'Melolo');
        ...
        if(details.recommendations && details.recommendations.length > 0) Core.renderCards(details.recommendations, 'recList'); 
        else Core.loadRecommendations();
    },
    ...
    playSpecificEpisode: async (index) => {
        ...
        const ep = Core.currentEpisodeList[index]; if(!ep) return;
        ...
        if (Core.currentSource === 'melolo' && !streamUrl) { 
            try { const fetchUrl = await Melolo.fetchStream(ep.vid); if(fetchUrl) streamUrl = fetchUrl; } catch(e) {} 
        }
    ...
    goBack: () => { 
        Core.stopPlayer(); // PENTING: Matikan video saat back
        history.back(); 
    },
...
function switchTab(t, p=true) {
    Core.stopPlayer(); // WAJIB: MATIKAN VIDEO SEBELUM GANTI HALAMAN
