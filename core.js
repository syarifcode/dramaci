const Core = {
    ...
    renderSkeleton: (containerId, count) => {
        ...
        for(let i=0; i<count; i++) {
            // Struktur HTML disamakan dengan card asli agar layout stabil
            html += `
    ...
    loadHome: async () => {
        ...
        // 1. FLICKREEL (Hero & Ranking)
        try {
            const flickData = await Flickreel.fetchForYou();
            
            // Hero Slider: Gabungan Karusel + Drama Laris
        ...
        // Ambil data dari kedua sumber secara bersamaan
        Promise.all([
            ...
            // Acak (Shuffle) agar terlihat seperti satu kesatuan platform
    ...
    // --- EXPLORE LOGIC (FIX DUPLICATE & SKELETON) ---
    loadExplore: async () => {
        ...
        // Logika Loading: Jika kosong pakai Skeleton, jika ada isi pakai Spinner
