const Core = {
    ...
    // --- VIP PAGE (REVERT KE DRAMABOX ONLY - BIAR CEPAT) ---
    loadVIP: async () => { 
        ...
        try {
            ...
            if (sections && sections.length > 0) {
                ...
                container.innerHTML = '<p style="text-align:center;width:100%">Gagal memuat VIP.</p>';
            }
        } catch(e) {
            container.innerHTML = '<p style="text-align:center;width:100%">Error memuat data.</p>';
        }
    },

    // --- SECTION PAGE (FIX BLANK & INFINITE FEEL) ---
    openSectionPage: async (provider) => {
        // 1. Pindah Tab Dulu biar Loading kelihatan
        ...
        try {
            if (provider === 'flick') {
                ...
                // TEKNIK JARIT: Ambil Home + Search 'CEO' + Search 'Cinta' (Total +/- 60 items)
                ...
                // TEKNIK JARIT: Ambil Home + Search 'Romantis' + Search 'Dendam'
                const [p1, p2, p3] = await Promise.all([
                    Melolo.fetchHome(),
                ...
                // Netshort datanya sudah banyak di satu endpoint
        ...
        gridEl.innerHTML = '';
        if (data.length > 0) {
            // Remove duplicates & shuffle sedikit biar variatif
    ...
    loadVIP: async () => { /* SUDAH DIUPDATE DI ATAS */ },
