const Core = {
    ...
    // ANTI BUG STATE (Mencegah Salah Video)
    activeFetchId: 0, 
    ...
    loadVIP: async () => {
        ...
        sections.forEach(sec => {
            ...
            title.style.borderLeft = "4px solid #E91E63";
    ...
    // --- PLAYER LOGIC (FIX BUG SALAH VIDEO) ---
    openPlayer: async (source, id) => {
        // 1. Buat ID Antrian Request
        ...
        video.src = "";
        video.poster = "";
        video.querySelectorAll('track').forEach(t => t.remove()); // Hapus subtitle lama
        ...
        // 5. Cek ID Antrian (Cegah balapan data)
        if (Core.activeFetchId !== thisRequestId) {
            console.log("Request drama kadaluarsa, dibatalkan.");
            return;
        }
    ...
    // Engine Putar Episode (UPDATE: Handle Subtitle)
    playSpecificEpisode: (index) => {
        ...
        // 2. Pasang Subtitle (Jika Ada)
        video.querySelectorAll('track').forEach(t => t.remove()); // Bersihkan dulu
        if (ep.subtitles && ep.subtitles.length > 0) {
            // Cari yg bahasa indo atau ambil yg pertama
