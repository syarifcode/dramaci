const Core = {
    ...
    // --- HOME LOGIC (PERFORMA TINGGI) ---
    loadHome: async () => {
        // 1. LOAD DRAMABOX DULUAN (Biar User Gak Nunggu)
        try {
        ...
        // 2. LOAD LATEST & NETSHORT (Paralel)
        ...
        // 3. LOAD MELOLO BELAKANGAN (Background Process - Inject to Banner)
        Melolo.fetchHome().then(mData => {
            const mCombined = [...(mData.trending||[]), ...(mData.latest||[])];
            
            if (mCombined.length > 0) {
                // A. Render List Melolo
                Core.renderCards(Core.removeDuplicates(mCombined), 'homeListMelolo', true);
                
                // B. SUNTIKKAN KE BANNER
                // Ambil 2 teratas Melolo, gabung ke Slider yg sudah ada
                const meloloTop = mCombined.slice(0, 2);
                const newSliderMix = [...Core.currentSliderData, ...meloloTop];
                newSliderMix.sort(() => Math.random() - 0.5); // Acak
                
                // Render Ulang Slider
                Core.renderSlider(newSliderMix.slice(0, 5));
            } else {
    ...
    // --- RENDER SLIDER (FIX: FALLBACK IMAGE) ---
    renderSlider: (list) => { 
        ...
        list.forEach(item => { 
            ...
            // FIX: Gunakan poster ATAU cover. Ini fix Melolo blank.
            const imgSrc = item.poster || item.cover;
            slide.innerHTML = `<img src="${imgSrc}" alt="${item.title}"><div class="hero-overlay"><div class="hero-info"><span class="hero-badge">Trending</span><h1>${item.title}</h1><p>${item.score}</p><button onclick="Core.openPlayer('${item.source}', '${item.id}')"><i class="fas fa-play"></i> Tonton</button></div></div>`; 
            container.appendChild(slide); 
        }); 
    },
    
    // UTILS Lainnya (Create Card, Share, Save, dll - Sama)
