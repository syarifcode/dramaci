const Core = {
    ...
    // --- SKELETON RENDERER (New) ---
    // Fungsi ini membuat kartu palsu yang berkedip sesuai jumlah yang diminta
    renderSkeleton: (containerId, count) => {
        ...
        for(let i=0; i<count; i++) {
            // Struktur HTML disamakan dengan .card asli agar layout tidak berantakan
            html += `
    ...
    // --- HOME LOGIC ---
    loadHome: async () => {
        // Tampilkan Skeleton dulu di semua section
        Core.renderSkeleton('homeListRank', 4);
        Core.renderSkeleton('homeListLatest', 4);
        Core.renderSkeleton('homeGridDB', 6);
        Core.renderSkeleton('homeListNS', 4);

        let flickFavs = []; 

        // 1. FLICKREEL (Hero, Ranking, & Fav)
        try {
            const flickData = await Flickreel.fetchForYou();
            
            // Hero Slider
            if (flickData.hero && flickData.hero.length > 0) {
                Core.currentSliderData = flickData.hero; 
            ...
            if (flickData.rank && flickData.rank.length > 0) {
                ...
                document.getElementById('homeListRank').innerHTML = ''; // Hapus skeleton
            ...
            // Simpan Fav
            if (flickData.fav) flickFavs = flickData.fav;

        } catch (e) { console.error("Flick Load Error", e); }

        // 2. MIXED LATEST (FLICKREEL + DRAMABOX)
        Promise.all([
            ...
            document.getElementById('homeListLatest').innerHTML = ''; // Hapus skeleton
        ...
        // 3. TRENDING MIXED
        DramaboxV2.fetchTrending().then(dbData => {
            let combinedTrending = [...(dbData || []), ...flickFavs];
            combinedTrending.sort(() => 0.5 - Math.random());
            
            document.getElementById('homeGridDB').innerHTML = ''; // Hapus skeleton

            if (combinedTrending.length > 0) {
                Core.renderCards(combinedTrending, 'homeGridDB');
            }
        });

        // 4. NETSHORT VIRAL
        Netshort.fetchTheaters().then(data => {
            document.getElementById('homeListNS').innerHTML = ''; // Hapus skeleton
        ...
        }).catch(() => document.getElementById('homeNetshortSection').style.display = 'none');
    },

    // --- EXPLORE LOGIC (With Skeleton) ---
    loadExplore: async () => {
        ...
        // Jika kosong (pertama kali buka), pakai Skeleton
        // Jika sudah ada isinya (infinite scroll), pakai spinner kecil di bawah
        if (container.children.length === 0) {
            Core.renderSkeleton('exploreGrid', 9); 
        } else {
        ...
        try {
            const [resNS, resFlick, resDB] = await Promise.allSettled([
                Netshort.fetchTheaters(), 
                Flickreel.fetchLatest(),  
                DramaboxV2.fetchLatest(Core.mixedPageCounter)
            ]);

            let combinedData = [];
            if (resNS.status === 'fulfilled' && resNS.value && resNS.value.viral) combinedData = [...combinedData, ...resNS.value.viral];
            if (resFlick.status === 'fulfilled' && Array.isArray(resFlick.value)) combinedData = [...combinedData, ...resFlick.value];
            if (resDB.status === 'fulfilled' && Array.isArray(resDB.value)) combinedData = [...combinedData, ...resDB.value];

            const existingIds = Array.from(container.querySelectorAll('.card')).map(c => c.dataset.id);
            let uniqueData = combinedData.filter(item => !existingIds.includes(item.id.toString()));
            uniqueData.sort(() => 0.5 - Math.random());

            // Hapus semua skeleton atau spinner SEBELUM render kartu baru
            // Kita cari elemen dengan class 'skeleton-card' atau 'loading-spinner' lalu hapus
            const loaders = container.querySelectorAll('.skeleton-card, .loading-spinner');
            loaders.forEach(el => el.remove());

            if (uniqueData.length > 0) {
                Core.renderCards(uniqueData, 'exploreGrid');
    ...
    handleSearch: async () => {
        ...
        // Tampilkan Skeleton Search
        Core.renderSkeleton('searchGrid', 6);
    ...
    loadVIP: async () => { 
        ...
        try {
            ...
            } else { container.innerHTML = '<p style="text-align:center;width:100%;margin-top:20px;">Data VIP Kosong.</p>'; }
        } catch(e) { container.innerHTML = '<p style="text-align:center;width:100%;margin-top:20px;">Gagal memuat VIP.</p>'; }
    },
    ...
    loadDubbed: async () => {
        ...
        Core.renderSkeleton('dubbedGrid', 6);
    ...
    playSpecificEpisode: async (index) => {
        ...
        if (index < Core.currentEpisodeList.length - 1) { nextBtn.disabled = false; nextBtn.innerHTML = `Lanjut <i class="fas fa-forward"></i>`; } 
        else { nextBtn.disabled = true; nextBtn.innerHTML = `Tamat <i class="fas fa-check"></i>`; }
    ...
    downloadCurrentEpisode: async (url, filename, btnElement) => {
        ...
        try {
            ...
            setTimeout(() => { btnElement.innerHTML = originalHtml; btnElement.disabled = false; }, 3000);
        } catch (e) { console.error("DL Error:", e); window.open(url, '_blank'); btnElement.innerHTML = originalHtml; btnElement.disabled = false; }
    },
    shareDrama: () => { 
    ...
    handleSearch: async () => {
        ...
        try {
            // Paralel Search: Dramabox + Netshort + Flickreel
            const [resDB, resNS, resFlick] = await Promise.allSettled([
                DramaboxV2.search(keyword),
                Netshort.search(keyword),
                Flickreel.search(keyword)
