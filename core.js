const Core = {
    ...
    // KEYS LOCALSTORAGE
    historyKey: 'dramaci_v10_history', // Ganti key biar fresh
    ...
    autoNextTimer: null,
    activeFetchId: 0,
    currentSource: null, // 'db', 'ns', 'melolo'
    currentDramaTitle: "", 
    ...
    // --- HOME LOGIC (TRIPLE SOURCE) ---
    loadHome: async () => {
        // 1. Load Melolo (Trending & New) - Paling atas biar fresh
        const meloloData = await Melolo.fetchHome();
    ...
    loadExplore: async () => {
        ...
        // Explore pakai Netshort dulu (Melolo belum ada endpoint explore random)
    ...
    loadDubbed: async () => {
        ...
        if(container.children.length > 1) return; // Jangan load kalau sudah ada isinya
        
        // Gabung Dubbing Dramabox & Netshort
    ...
    handleSearch: async () => {
        ...
        // Panggil 3 Provider sekaligus (Parallel Processing)
    ...
    openPlayer: async (source, id) => {
        ...
        document.getElementById('detailDesc').innerText = details.intro || "Tidak ada sinopsis.";
        ...
        details.episodes.forEach((ep, index) => {
            ...
            // Bersihkan nama episode biar rapi
            const cleanName = ep.name.replace('Episode ', '').replace('EP ', '');
    ...
    playSpecificEpisode: async (index) => {
        ...
        // --- LOGIKA PEMUTARAN BERDASARKAN SUMBER ---
        let streamUrl = ep.url;

        // Khusus MELOLO: Ambil link JIT (Just In Time)
        if (Core.currentSource === 'melolo') {
            // Tampilkan loading di video kalau bisa (optional UI improvement)
            const fetchUrl = await Melolo.fetchStream(ep.vid);
            if(fetchUrl) streamUrl = fetchUrl;
            else { alert("Gagal mengambil link video Melolo."); return; }
        }

        // Khusus NETSHORT: Butuh CORS & Subtitle
        if (Core.currentSource === 'ns') video.setAttribute('crossorigin', 'anonymous');
        else video.removeAttribute('crossorigin');

        video.src = streamUrl;
        
        // Handle Subtitles (Netshort only)
        video.querySelectorAll('track').forEach(t => t.remove());
    ...
    loadRecommendations: async () => {
        // Ambil rekomendasi acak dari Netshort (paling stabil buat feed)
        const nsData = await Netshort.fetchTheaters();
    ...
    toggleSave: () => {
        ...
        const existingIndex = savedList.findIndex(item => item.id == Core.currentDramaDetails.id); // Loose compare
    ...
    saveToHistory: (item) => {
        ...
        history = history.filter(h => h.id != item.id); // Loose compare
    ...
    loadHistory: () => {
        ...
        else container.innerHTML = `<p style="text-align:center;width:100%;color:#666;margin-top:20px;">${emptyMsg}</p>`;
    },

    clearHistory: () => {
        if (Core.activeHistoryTab === 'history') {
            if(confirm("Hapus semua riwayat tontonan?")) { localStorage.removeItem(Core.historyKey); Core.loadHistory(); }
        } else {
            if(confirm("Hapus semua drama yang disimpan?")) { localStorage.removeItem(Core.savedKey); Core.loadHistory(); }
        }
    },
