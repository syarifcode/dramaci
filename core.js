const Core = {
    ...
    mixedPageCounter: 1, 
    currentSliderData: [], // Simpan data slider sementara
    ...
    // --- HOME LOGIC (BANNER FIX) ---
    loadHome: async () => {
        // 1. LOAD DRAMABOX UTAMA (Cepat)
        try {
            const dbData = await DramaboxV2.fetchTrending();
            if (dbData && dbData.length > 0) {
                Core.currentSliderData = dbData.slice(0, 5); // Ambil 5 Awal
                Core.renderSlider(Core.currentSliderData); // Render Langsung
                Core.renderCards(dbData, 'homeGridDB'); // Render List Bawahnya
            }
        } catch (e) { console.error("DB Load Error", e); }

        // 2. LOAD MELOLO (Inject ke Banner jika ada)
        Melolo.fetchHome().then(mData => {
            const mCombined = [...(mData.trending||[]), ...(mData.latest||[])];
            
            if (mCombined.length > 0) {
                // Render List Melolo Horizontal
                Core.renderCards(Core.removeDuplicates(mCombined), 'homeListMelolo', true);

                // LOGIKA BANNER BARU:
                // Ambil 2 dari Melolo, gabungkan dengan yang sudah ada di slider
                // Pastikan tidak mereset slider jadi kosong
                const meloloTop = mCombined.slice(0, 2);
                const current = Core.currentSliderData.length > 0 ? Core.currentSliderData : [];
                
                // Gabung: [Melolo 1, DB 1, DB 2, Melolo 2, DB 3]
                const mixed = [...meloloTop, ...current].slice(0, 7); // Max 7 slide
            ...
            // Kalau Melolo error, biarkan slider Dramabox yang sudah tampil
            document.getElementById('homeMeloloSection').style.display = 'none';
        });

        // 3. Load Latest & Netshort (Standard)
    ...
    loadDubbed: async () => {
        ...
        try {
            // FIX: Gunakan allSettled agar kalau satu error, halaman TIDAK BLANK
            ...
            // Ambil hasil Dramabox Page 1
            if (results[0].status === 'fulfilled' && Array.isArray(results[0].value)) {
                combined = [...combined, ...results[0].value];
            }
            // Ambil hasil Dramabox Page 2
            if (results[1].status === 'fulfilled' && Array.isArray(results[1].value)) {
                combined = [...combined, ...results[1].value];
            }
            // Ambil hasil Netshort (Dubbing)
            if (results[2].status === 'fulfilled' && results[2].value && results[2].value.dubbing) {
                combined = [...combined, ...results[2].value.dubbing];
            }
    ...
    // --- EXPLORE LOGIC (INFINITY NETSHORT V11) ---
    loadExplore: async () => {
    ...
    renderSlider: (list) => { 
        ...
        list.forEach(item => { 
            ...
            // Fallback image penting biar gak blank
