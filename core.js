const Core = {
    ...
    // --- VIP PAGE (REVERT KE DRAMABOX ONLY - SUPAYA CEPAT) ---
    loadVIP: async () => { 
        ...
        // MURNI DRAMABOX Fetching (Tanpa campuran Flickreels)
        Dramabox.fetchVIP().then(sections => {
            ...
            if (sections && sections.length > 0) {
                sections.forEach(sec => { 
                    ...
                    w.id='vip-'+Math.random().toString(36).substr(2,9);
                    w.className='grid-container'; w.style.gridColumn="1/-1"; 
                    container.appendChild(w); 
                    
                    // Render Kartu ke dalam Wrapper
                    Core.renderCards(sec.list, w.id);
                });
            } else {
                container.innerHTML = '<p style="text-align:center;width:100%">Data VIP Kosong.</p>';
            }
        }).catch(e => {
            console.error(e);
            container.innerHTML = '<p style="text-align:center;width:100%">Gagal memuat data VIP.</p>';
        });
    },

    // --- SECTION PAGE (VIEW ALL - INFINITE FEEL) ---
    openSectionPage: async (provider) => {
        ...
        try {
            if (provider === 'flick') {
                ...
                // Ambil Home + Search 'CEO' + Search 'Cinta'
            ...
            else if (provider === 'melolo_all') {
                titleEl.innerText = "Melolo Untukmu";
                // Ambil Home + Search 'Romantis' + Search 'Dendam'
                const [p1, p2, p3] = await Promise.all([
                    Melolo.fetchHome(),
                    Melolo.search("Romantis"),
                    Melolo.search("Dendam")
                ]);
                data = [...(p1.trending||[]), ...(p1.latest||[]), ...p2, ...p3];
            }
            else if (provider === 'ns') {
                titleEl.innerText = "Netshort Viral";
                const res = await Netshort.fetchTheaters();
                data = [...(res.viral||[]), ...(res.dubbing||[])];
            }
        } catch (e) { console.error("Fetch Error", e); }

        gridEl.innerHTML = '';
        if (data.length > 0) {
            const uniqueData = Core.removeDuplicates(data);
            Core.renderCards(uniqueData, 'sectionGrid');
        } else {
            gridEl.innerHTML = '<p style="text-align:center;width:100%;margin-top:20px;color:#666;">Gagal memuat data.</p>';
        }
    },
    ...
    renderSlider: (list) => {
        ...
        top5.forEach(item => {
            ...
            slide.innerHTML = `<img src="${bgUrl}" alt="${item.title}"><div class="hero-overlay"><div class="hero-info"><span class="hero-badge">Flickreels</span><h1>${item.title}</h1><p>${item.score}</p><button onclick="Core.openPlayer('${item.source}', '${item.id}')"><i class="fas fa-play"></i> Tonton</button></div></div>`;
