const Core = {
    ...
    // UPDATE: State untuk Auto Slider
    sliderInterval: null, 
    ...
    // --- HOME LOGIC (MIXED VERSION - FIXED BANNER) ---
    loadHome: async () => {
        // 1. HERO SLIDER & RANKING (DENGAN FALLBACK)
        try {
            const flickData = await Flickreel.fetchForYou();
            
            // --- LOGIKA BARU: Cek & Fallback untuk Banner ---
            let heroData = [];
            
            // Cek apakah Flickreel punya data hero
            if (flickData.hero && flickData.hero.length > 0) {
                heroData = flickData.hero;
            } else {
                // FALLBACK: Jika kosong, ambil 5 drama trending dari Dramabox
                // Ini mencegah loading berputar selamanya
                const dbTrending = await DramaboxV2.fetchTrending();
                if (dbTrending && dbTrending.length > 0) {
                    heroData = dbTrending.slice(0, 5);
                }
            }

            const sliderContainer = document.getElementById('heroSlider');

            // UPDATE: Render Slider atau Sembunyikan (Fix Kotak Hitam)
            if (heroData.length > 0) {
                if(sliderContainer) sliderContainer.style.display = 'flex'; // Pastikan muncul
                Core.currentSliderData = heroData; 
                Core.renderSlider(Core.currentSliderData); 
            } else {
                // Hapus spinner dan sembunyikan container jika data kosong
                if(sliderContainer) {
            ...
            // Ranking List (Tetap seperti semula)
            ...
            // UPDATE: Pastikan spinner hilang dan container sembunyi kalau error
            const sliderContainer = document.getElementById('heroSlider');
            if(sliderContainer) {
                sliderContainer.innerHTML = '';
                sliderContainer.style.display = 'none';
    ...
    loadVIP: async () => { 
        ...
        if(container.children.length > 1) return; // Cache check
    ...
    loadDubbed: async () => {
        ...
        container.innerHTML = '<div class="loading-spinner"></div>';
        try {
            // Ambil beberapa sumber sekaligus
            ...
            if (combined.length > 0) { 
                ...
                Core.pageDubbed = 3; // Siap load page selanjutnya
            } else { 
    ...
    renderSlider: (list) => { 
        ...
        // UPDATE: Mulai Auto Scroll Slider
        Core.startHeroSlider();
    },

    // --- FUNGSI BARU: Logika Auto Scroll ---
    startHeroSlider: () => {
        const container = document.getElementById('heroSlider');
        if (!container) return;

        // Bersihkan interval lama jika ada (biar tidak tumpang tindih)
        if (Core.sliderInterval) clearInterval(Core.sliderInterval);

        // Set interval baru setiap 4 detik
        Core.sliderInterval = setInterval(() => {
            // Cek lebar scroll saat ini + lebar layar
            ...
            if (container.scrollLeft >= maxScroll - 10) {
                ...
                // Geser ke kanan sebesar lebar layar
                container.scrollBy({ left: container.clientWidth, behavior: 'smooth' });
            }
        }, 4000); // 4000ms = 4 detik

        // UX: Stop geser otomatis kalau user sedang sentuh banner
        container.addEventListener('touchstart', () => clearInterval(Core.sliderInterval));
        // UX: Jalan lagi kalau user lepas sentuhan
        container.addEventListener('touchend', () => Core.startHeroSlider());
    },
