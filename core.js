const Core = {
    ...
    handleSearch: async () => {
        ...
        const combined = [...meloloRes, ...nsRes, ...dbRes]; 
    ...
    // --- RENDER (DENGAN PROXY GAMBAR MELOLO) ---
    renderCards: (list, containerId, isHorizontal = false) => {
        ...
        list.forEach(item => {
            ...
            if(item.source === 'melolo') {
                // Pakai wsrv.nl karena HTML tidak boleh ada meta no-referrer
    ...
    openPlayer: async (source, id) => {
        ...
        // Proxy Poster Melolo
        if(source === 'melolo') video.poster = `https://wsrv.nl/?url=${encodeURIComponent(details.cover)}&output=webp`;
        else video.poster = details.cover;
    ...
    playSpecificEpisode: async (index) => {
        ...
        // KHUSUS MELOLO: FETCH STREAM URL
