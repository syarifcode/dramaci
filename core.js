const Core = {
    ...
    loadHome: () => {
        ...
        Melolo.fetchHome().then(meloloData => {
            if (meloloData && meloloData.trending && meloloData.trending.length > 0) {
            ...
            if (meloloData && meloloData.latest && meloloData.latest.length > 0) {
    ...
    loadDubbed: async () => {
        const container = document.getElementById('dubbedGrid'); if(container.children.length > 1) return;
    ...
    handleSearch: async () => {
        ...
        const results = await Promise.allSettled([ Dramabox.search(keyword), Netshort.search(keyword), Melolo.search(keyword) ]);
        const combined = [...(results[2].status==='fulfilled'?results[2].value:[]), ...(results[1].status==='fulfilled'?results[1].value:[]), ...(results[0].status==='fulfilled'?results[0].value:[])];
    ...
    openPlayer: async (source, id) => {
        ...
        // SATPAM 1: Cek apakah user sudah pindah halaman?
        if (Core.activeFetchId !== thisReq) return; 
        if (document.getElementById('playerPage').style.display === 'none') return; // FIX SUARA HANTU
        ...
        if(source === 'melolo') video.poster = `https://wsrv.nl/?url=${encodeURIComponent(details.cover)}&output=webp`; else video.poster = details.cover;
    ...
    playSpecificEpisode: async (index) => {
        ...
        // UI Update (Biar user tau lagi loading)
        ...
        if (Core.currentSource === 'melolo') {
            try {
                // Fetch URL Stream (Async)
                const fetchUrl = await Melolo.fetchStream(ep.vid);
                if(fetchUrl) streamUrl = fetchUrl;
                else { alert("Gagal mengambil link video."); return; }
            } catch(e) { alert("Error Stream Melolo"); return; }
        }

        // SATPAM 2: CEK LAGI! APAKAH PLAYER MASIH DIBUKA?
        // Kalau user sudah tekan Back saat loading tadi, STOP DISINI.
